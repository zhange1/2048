class Game {
  ...
  newItem(arr) {
    ...
    arr[noNum[item].out][noNum[item].inner] = _this.max[newNumIndex]
  ...
  moveLeft(arr) {
    ...
    for (let indexs = 0; indexs < 4; indexs++) {
      for (let index = 0; index < 3; index++) {
        if(!_this.max.includes(arr[indexs][index])){
          _this.max.push(arr[indexs][index])
        }
        if (index < 3) { //当前index >0，则判断其左侧是否有值，不然，则判断当前值
          [arr[indexs][index], arr[indexs][index + 1]] = _this.itemCount(arr[indexs][index], arr[indexs][index +1])
          if (_this.isMove) {
            indexs = 0
            index = -1
            _this.isMove = false
  ...
  moveTop(arr) {
    ...
    for (let indexs = 0; indexs < 3; indexs++) {
      for (let index = 0; index < 4; index++) {
        ...
        [arr[indexs][index], arr[indexs + 1][index]] = _this.itemCount(arr[indexs][index], arr[indexs + 1][index])
        if (_this.isMove) {
          indexs = 0
          index = -1
          _this.isMove = false
  ...
  */
  ...
  moveLeft(arr) {
    ...
    for (let indexs = 0; indexs < 4; indexs++) {
      for (let index = 0; index < 3; index++) {
        if (!_this.max.includes(arr[indexs][index]['value'])) {
